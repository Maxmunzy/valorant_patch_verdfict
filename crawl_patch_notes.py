...
PATCHES = [
    ...
    # ── E10 ──────────────────────────────────────────────────────────────────
    {"version": "9.03",  "act": "V25A1", "url": "valorant-patch-notes-9-03"},
    {"version": "9.04",  "act": "V25A1", "url": "valorant-patch-notes-9-04"},
    {"version": "9.05",  "act": "V25A1", "url": "valorant-patch-notes-9-05"},
    {"version": "9.07",  "act": "V25A1", "url": "valorant-patch-notes-9-07"},
    {"version": "9.08",  "act": "V25A2", "url": "valorant-patch-notes-9-08"},
    {"version": "9.09",  "act": "V25A2", "url": "valorant-patch-notes-9-09"},
    {"version": "9.10",  "act": "V25A2", "url": "valorant-patch-notes-9-10"},
    {"version": "9.11",  "act": "V25A3", "url": "valorant-patch-notes-9-11"},
    # ── E11 ──────────────────────────────────────────────────────────────────
    {"version": "10.01", "act": "V25A4", "url": "valorant-patch-notes-10-01"},
    {"version": "10.02", "act": "V25A4", "url": "valorant-patch-notes-10-02"},
    {"version": "10.03", "act": "V25A4", "url": "valorant-patch-notes-10-03"},
    {"version": "10.04", "act": "V25A4", "url": "valorant-patch-notes-10-04"},
    {"version": "10.05", "act": "V25A4", "url": "valorant-patch-notes-10-05"},
    {"version": "10.06", "act": "V25A4", "url": "valorant-patch-notes-10-06"},
    {"version": "10.08", "act": "V25A5", "url": "valorant-patch-notes-10-08"},
    {"version": "10.09", "act": "V25A5", "url": "valorant-patch-notes-10-09"},
    {"version": "10.10", "act": "V25A5", "url": "valorant-patch-notes-10-10"},
    {"version": "10.11", "act": "V25A5", "url": "valorant-patch-notes-10-11"},
    # ── E12 / V26 ────────────────────────────────────────────────────────────
    {"version": "11.01", "act": "V25A6", "url": "valorant-patch-notes-11-01"},
    {"version": "11.02", "act": "V25A6", "url": "valorant-patch-notes-11-02"},
    {"version": "11.04", "act": "V25A6", "url": "valorant-patch-notes-11-04"},
    {"version": "11.05", "act": "V25A6", "url": "valorant-patch-notes-11-05"},
    {"version": "11.06", "act": "V26A1", "url": "valorant-patch-notes-11-06"},
    {"version": "11.07", "act": "V26A1", "url": "valorant-patch-notes-11-07"},
    {"version": "11.08", "act": "V26A1", "url": "valorant-patch-notes-11-08"},
    {"version": "11.09", "act": "V26A1", "url": "valorant-patch-notes-11-09"},
    {"version": "11.10", "act": "V26A1", "url": "valorant-patch-notes-11-10"},
    {"version": "11.11", "act": "V26A1", "url": "valorant-patch-notes-11-11"},
...
def infer_direction(text: str, section_header: str) -> str:
    """
    맥락 인식 direction 추론.
    수치 변화 → 고유스킬/슬롯 변경 → 키워드 순으로 판단.
    - cost/price/credits/cooldown/recharge: 감소=buff, 증가=nerf
    - duration/damage/range/radius/health/charge: 증가=buff, 감소=nerf
    - 고유스킬(signature) 지정 = buff (무료), 해제 = nerf (유료화)
    """
    import re
    t = (text + " " + section_header).lower()

    # ── 1단계: 수치 변화 패턴 "X >>> Y" 또는 "from X to Y" ─────────────────
    # 단위 접미사(s, m, %, hp 등)를 허용하여 "1.1s >>> 1.5s" 같은 패턴도 캡처
    arrow = re.search(r'(\d[\d.]*)[a-z%/]*\s*>>>\s*(\d[\d.]*)', t)
    from_to = re.search(r'from\s+(\d[\d.]*)[a-z%/]*\s+to\s+(\d[\d.]*)', t)
    m = arrow or from_to
    if m:
        ...
        # 비용/대기시간 stat: 감소=buff, 증가=nerf
        # (cost/cooldown 계열 + 각종 time/delay/windup 계열)
        cost_ctx = any(w in t for w in [
            "cost", "credit", "price",
            "cooldown", "recharge", "recovery time",
            "arm time", "deploy time", "cast time", "setup time",
            "equip time", "unequip time", "equip delay", "unequip delay",
            "windup", "wind-up", "wind up",
            "resurrection time", "revive time", "res time",
            "delay",
            "energy drain", "fuel drain", "battery drain",
        ])
        if cost_ctx:
            return "buff" if decreased else ("nerf" if increased else "neutral")

        # 그 외(데미지/사거리/지속시간/체력): 증가=buff, 감소=nerf
        return "buff" if increased else ("nerf" if decreased else "neutral")

    # "Free >>> X" 패턴: 무료에서 유료로 전환 = nerf
    if re.search(r'free\s*>>>\s*\d', t):
        return "nerf"
    # "X >>> Free" 패턴: 유료에서 무료로 전환 = buff
    if re.search(r'\d[a-z%]*\s*>>>\s*free', t):
        return "buff"

    # ── 2단계: 고유스킬(signature) 슬롯 변경 ──────────────────────────────
    # 고유스킬이 됨 = 무료 → buff
    sig_buff = [
        "changed to a signature ability",
        "changed to signature ability",
        "changed to the signature ability",
        "moved to the signature slot",
        "moved to signature slot",
        "is now the signature ability",
        "is now a signature ability",
        "now a signature ability",
        "now the signature ability",
    ]
    # 고유스킬 해제 = 유료화 → nerf
    sig_nerf = [
        "changed to a non-signature ability",
        "changed to a basic ability",
        "changed to non-signature",
        "changed to basic ability",
        "moved to basic slot",
        "is now a basic ability",
        "now a basic ability",
        "no longer a signature",
    ]
    if any(p in t for p in sig_buff):
        return "buff"
    if any(p in t for p in sig_nerf):
        return "nerf"

    # ── 3단계: 비용/충전 구조 변경 ─────────────────────────────────────────
    # 무료 충전 / 킬 리차지 = buff
    if any(p in t for p in ["free charge", "free on round start", "no longer costs",
                             "now free", "is free", "kill recharge", "kill to recharge",
                             "grants a free charge"]):
        return "buff"
    # 유료 전환 / 충전 제거 = nerf
    if any(p in t for p in ["no longer free", "now costs", "removed the free", "removed free charge"]):
        return "nerf"
    # 새 쿨다운 추가 = nerf ("now incurs a cooldown", "will now always incur a cooldown")
    if re.search(r'(now|will)\s+\w*\s*(always\s+)?incur\s+a\s+\w*\s*cooldown', t):
        return "nerf"
    if re.search(r'now\s+has\s+a\s+\w*\s*cooldown', t):
        return "nerf"

    # ── 4단계: "no longer" 패턴 — 맥락 구분 ───────────────────────────────
    if "no longer" in t:
        # "no longer has a cooldown" / "no longer costs" = buff
        if any(p in t for p in ["no longer has a cooldown", "no longer costs",
                                 "no longer require", "no longer penaliz"]):
            return "buff"
        # 그 외 "no longer" = 기능 제거 → nerf
        return "nerf"

    # ── 5단계: "now always" 패턴 ───────────────────────────────────────────
    # "now always kill/deal/activate/..." = 신뢰성 향상 → buff
    if re.search(r'now\s+always\s+(kill|deal|trigger|activate|apply|work|function)', t):
        return "buff"
    ...
    buff_signals = ["improved", "added charges", "new charge", "now can", "lower cost",
                    "reduced cooldown", "decreased cooldown", "faster", "increased range",
                    "increased damage", "increased duration", "increased radius"]
    nerf_signals = ["removed", "increased cooldown", "higher cost", "slower",
                    "decreased damage", "decreased duration", "decreased range",
                    "decreased radius", "reduced damage", "reduced duration",
                    "reduced range", "reduced radius"]

    if cost_ctx:
        # 비용/쿨다운 맥락: increased=nerf, decreased=buff
        if "increased" in t and "decreased" not in t:
            return "nerf"
        if "decreased" in t and "increased" not in t:
            return "buff"

    is_buff = any(w in t for w in buff_signals)
    is_nerf = any(w in t for w in nerf_signals)

    if is_buff and not is_nerf:
        return "buff"
    if is_nerf and not is_buff:
        return "nerf"
    return "neutral"
...
def extract_patch_date(html: str) -> str | None:
    """패치 노트 HTML에서 배포 날짜 추출 → 'YYYY-MM-DD' 문자열 반환"""
    soup = BeautifulSoup(html, "html.parser")

    # 1) <time datetime="..."> 태그
    time_tag = soup.find("time", attrs={"datetime": True})
    if time_tag:
        raw = time_tag["datetime"]  # e.g. "2026-04-01T10:00:00Z"
        try:
    ...
    if not meta:
        meta = soup.find("meta", attrs={"name": "publish_date"})
...
def crawl_patch(page, patch: dict) -> tuple[list[dict], str | None]:
    """패치 노트 크롤링 → (변경 행 목록, 배포 날짜 'YYYY-MM-DD')"""
    ...
    patch_date = extract_patch_date(html)
    print(f"{len(rows)}건  날짜: {patch_date or '미확인'}")
    return rows, patch_date
...
def main():
    ...
    all_rows = []
    date_updates: dict[str, str] = {}   # version → "YYYY-MM-DD"
    ...
    with sync_playwright() as p:
        ...
        for patch in targets:
            rows, patch_date = crawl_patch(page, patch)
            all_rows.extend(rows)
            if patch_date:
                date_updates[patch["version"]] = patch_date
            time.sleep(1)

        browser.close()

    # ── patch_dates.json 업데이트 ─────────────────────────────────────────────
    import os, json
    dates_path = "patch_dates.json"
    existing_dates: dict[str, str] = {}
    if os.path.exists(dates_path):
        with open(dates_path, encoding="utf-8") as f:
            existing_dates = json.load(f)
    existing_dates.update(date_updates)
    with open(dates_path, "w", encoding="utf-8") as f:
        json.dump(existing_dates, f, ensure_ascii=False, indent=2)
    if date_updates:
        print(f"\n날짜 저장: {dates_path}  ({len(date_updates)}개 추가/갱신)")
