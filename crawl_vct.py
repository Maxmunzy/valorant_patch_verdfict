...
def crawl_tournament(page, tournament: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for i, mid in enumerate(match_ids, 1):
        ...
        n_maps = len(set(r["map"] for r in rows))
        print(f"  [{i:3d}/{len(match_ids)}] 매치 {mid}  {n_maps}맵  {len(rows)}행")
        time.sleep(0.8)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for tournament in targets:
            _, summary = crawl_tournament(page, tournament)
