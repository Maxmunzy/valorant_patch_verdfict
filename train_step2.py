...
def collapse_label(label):
    ...
    방향(nerf/buff) × 컨텍스트(first/followup/correction/rework) 기준
    """
    if label == "stable_balanced":  return "stable"
    if label == "stable_strong":    return "stable_strong"
    if label == "stable_weak":      return "stable_weak"
    if label == "stable_rank_only": return "stable_rank_only"  # 랭크 편향 설계 요원
    if label == "stable":           return "stable"
    if label == "rework":          return "rework"
    if label.startswith("correction_buff"): return "correction_buff"
    if label.startswith("correction_nerf"): return "correction_nerf"

    parts     = label.split("_")
    direction = parts[0]
    is_followup = label.endswith("followup")
    is_pro      = any(t in label for t in ("pro_dom", "role_inv", "skill_ceil"))

    if direction == "nerf":
        if is_followup: return "nerf_followup"
        if is_pro:      return "nerf_pro"
        return "nerf_rank"
    if direction == "buff":
        if is_followup: return "buff_followup"
        if is_pro:      return "buff_pro"
        return "buff_rank"
...
DROP_COLS = [
    ...
    "last_trigger_type",                        # rank 쏠림(599/669)으로 변별력 없음, 양 Stage SHAP=0
...
def prepare(df):
    ...
    feat_cols = [c for c in df.columns if c not in DROP_COLS + ["label_collapsed"]]
    return df, feat_cols
...
def main():
    ...
    df, feat_cols = prepare(pd.read_csv("step2_training_data.csv"))
    print(f"전체: {len(df)}행 / 피처: {len(feat_cols)}개")
    print("\n[collapsed 레이블 분포]")
    ...
    if ba_a_lr > ba_a_xgb:
        ...
        shap_top(model_a_xgb, X_all, feat_cols, "Stage A (stable vs patched)", save_path="shap_importance_a.csv")
    ...
    X_b          = patched_df[feat_cols].values.astype(np.float32)
    ...
    if ba_b_lr > ba_b_xgb:
        ...
        shap_top(model_b_xgb, X_b, feat_cols, "Stage B (patch type)", save_path="shap_importance_b.csv")
    ...
    prob_patch = model_a.predict_proba(X_now)[:, 1]
    prob_b_all = model_b.predict_proba(X_now)   # (n_agents, n_classes)
    pred_b_raw = model_b.predict(X_now)
    ...
    joblib.dump({"model_a": model_a, "model_b": model_b,
                 "feat_cols": feat_cols, "label_b_cats": label_b_cats},
                "step2_pipeline.pkl")
    ...
    print("="*50)
    loao_cv(df, feat_cols, stable_types, stage="A")
    loao_cv(df, feat_cols, stable_types, stage="B")
